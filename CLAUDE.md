...
**DB (`supabase/migrations/1016_auto_cancel_stale_payments.sql`) — written, NOT applied.** No Supabase MCP tool this session (same constraint as 1014/1015); the user needs to run this against both DEV and PROD via the SQL Editor, same as those two.
...
**Verification, and its honest limit — same recurring pattern as 1014/1015**: `tsc --noEmit` / `eslint .` / `npm run build` all clean (throwaway placeholder `.env.local`, deleted immediately, never committed). **No live-network verification was possible** — this session has no Supabase MCP tool, so the CHECK-constraint lookup, the function body, and the `pg_cron` scheduling have never been executed against a real database. Per this project's own two "Production incident" write-ups, a migration is not verified until it's actually run — the constraint-name lookup in particular is the kind of thing that's easy to get subtly wrong (e.g. if `pg_get_constraintdef` doesn't match `%status%` for some reason and the DO block silently no-ops, leaving the old 4-value constraint in place and the later `alter table ... add constraint` failing outright since the name would collide, or the old constraint still rejecting `'cancelled'` if the ADD happened to succeed anyway). **The user should watch for errors on first run of this migration in the SQL Editor** and report back if the constraint-swap step doesn't behave as described, rather than assuming success. Once applied, confirm it worked by checking `select * from cron.job;` (the schedule exists) and, after 15+ minutes, `select * from cron.job_run_details order by start_time desc limit 5;` (it's actually firing) — then re-check the Steel Fitness-style stuck order is gone within an hour of going stale.
