...
## Milestone: Auto-cancel stale Razorpay orders (2026-09-19)

Triggered by a real production incident: Steel Fitness had a ₹599 Razorpay order stuck at `status='created'` forever (an abandoned checkout — the gym owner opened it, never completed it), cleared by hand via a one-off SQL script since no system existed to resolve it. User's direction: "any payment which has been cancelled should be marked as cancelled, build this system for prod" — asked whether that should be (a) subscription-cancel-cascades-to-its-payments, (b) stuck/abandoned orders auto-expiring, or (c) a manual admin button; user chose (b) only.

**DB (`supabase/migrations/1016_auto_cancel_stale_payments.sql`) — applied to both DEV and PROD via the SQL Editor, user-run and user-confirmed 2026-09-19.** No Supabase MCP tool this session (same constraint as 1014/1015), so this session itself could not run it — the user pasted the script into both projects directly.
- `platform_payments.status` widened from 4 values to 5 (`cancelled` added) — its CHECK constraint's real name isn't known from this repo (the table itself is defined in FitDeskApp's migrations), so the migration looks it up dynamically by inspecting `pg_constraint`/`pg_get_constraintdef` for the one mentioning `status`, rather than guessing a name.
- `expire_stale_payment_orders()` — flips any `'created'` row older than 1 hour to `'cancelled'` (reusing the exact threshold `admin_billing_pipeline`/`admin_overview_stats` already used for their "orders stuck as created" metric, rather than inventing a second definition of "stale"), tagging `provider_metadata` with `auto_cancelled: true` and inserting one `admin_audit_log` row per payment with `admin_id = null` (renders as "System" — `features/gyms/audit.ts` already had that fallback for a null admin_id). **Deliberately not admin-gated and not granted to any client role** — this must never be reachable via `supabase.rpc(...)`, only by the scheduled job below or a superuser running it by hand in the SQL editor.
- A `pg_cron` job (`expire-stale-platform-payments`, every 15 minutes) calls it. Scheduling is idempotent (unschedules any existing job of the same name first), so re-running this file — or applying it to DEV and PROD from the same script — never duplicates the job. If `create extension pg_cron` fails with a permissions error, the migration's own header says to enable it via the Dashboard's Database → Extensions page first.
- Once this runs regularly, the pre-existing "orders stuck as created" tiles (Overview's billing pipeline panel, `admin_overview_stats`) should settle near zero within an hour of any stuck order appearing — that metric was always a symptom this migration is the actual fix for, not a separate thing to reconcile.

**App:** `InvoiceStatus` (`revenue/mock-data.ts`) gains `"Cancelled"`; both `STATUS_MAP`s (`revenue/queries.ts`, `features/gyms/billing.ts`) map `cancelled` → `"Cancelled"`; the Gym Detail billing tab's status filter gained a Cancelled option. No new pill tone needed — `core/ui/status-style.ts` already had a muted `Cancelled` tone for subscription status, and it reads correctly for a cancelled payment too (documented inline as now serving both vocabularies).

**Verification.** `tsc --noEmit` / `eslint .` / `npm run build` all clean before handing the script over (throwaway placeholder `.env.local`, deleted immediately, never committed). This session had no Supabase MCP tool, so the SQL itself was never run from here — but unlike 1014/1015, the gap was closed the same day: the user ran the full script directly in the SQL Editor on **both** projects and reported back the actual output at each step, not just "it worked":
- The constraint-lookup `do $$...$$` block ran without error on both (the dynamic `pg_get_constraintdef` name lookup worked as designed — a real risk this migration's own header flagged, now confirmed moot).
- `select * from cron.job;` showed the job registered identically on both (`jobid: 1`, `schedule: */15 * * * *`, `command: select public.expire_stale_payment_orders();`) — PROD project name shows as "MyFitDesk", DEV as "siddiq786001@gmail.com's Project" (its Supabase default name, never renamed — confirmed by the user to be the actual DEV project this app's environment switch points at, not a stray unrelated project).
- `select * from cron.job_run_details ...` showed a real `succeeded` run on PROD (`runid: 1`) at the 15-minute mark, and the user confirmed the same on DEV after waiting for the next tick.

This is the first of the three 1014/1015/1016 migrations in this file's "written but not applied" streak to get a real execution check the same session it was written, closing exactly the gap the other two still carry. **Still not checked**: an actual row landing in `platform_payments` with `status='cancelled'` from a real stale order (both confirmations above are "the job runs successfully", not yet "the job did something" — a `succeeded` run with zero eligible rows looks identical in `cron.job_run_details`) — worth a `select ... where status = 'cancelled'` check the next time a real abandoned checkout sits stale for over an hour on either project.

---

## Milestone: Manual/cash payments on Extend + real discounted prices on Change package (2026-09-19)

Two real gaps the product owner hit using the live "Manage subscription" sheet on a real gym: (1) Extend had no way to record that the extension was actually paid for — a gym owner paying cash in person left zero trace in `platform_payments`, the sheet only ever moved `current_period_end`; (2) "Change package" listed every package's raw `price_minor`, silently ignoring any `plan_offers` discount on a dynamic plan cycle (Milestone: Dynamic Plans) — an admin couldn't see the price they'd actually discounted a gym into.

**DB (`supabase/migrations/1014_admin_manual_payment.sql`) — applied to both DEV and PROD via the SQL Editor directly (user-run; no Supabase MCP tool in this session, so this session could not independently verify the apply or re-run the live-`.rpc()`-call verification style every other migration in this file has used — see those milestones' own "Production incident" write-ups for why that step matters and is still owed here).**
- `platform_payments` gains a nullable `method` column (`cash`/`upi`/`bank_transfer`/`card`/`cheque`/`other`, CHECK-constrained) — mirrors the tenant `payments` table's own `method` column; always null for a razorpay row (the gateway itself is that row's "method").
- `admin_extend_subscription` (1004) gains three optional trailing args (`p_payment_amount_minor`/`p_payment_method`/`p_payment_note`) — dropped and recreated (signature change) rather than `CREATE OR REPLACE`, same convention as every other signature-widening change in this file. When an amount is given, it inserts a `platform_payments` row (`provider='manual'`) in the *same transaction* as the date extension, so an extension and its payment record can never end up out of sync — and a second `admin_audit_log` row (`payment.manual_record`) alongside the existing `subscription.extend` one.
  - **Open, unresolved product decision**: the manual-entry `invoice_number` is a placeholder (`MANUAL-<timestamp>-<random>`), invented because this repo has no visibility into FitDeskApp's real invoice-numbering sequence (`next_platform_invoice_number`, referenced only in a comment — see `revenue/mock-data.ts`). Flagged to the user; not yet resolved to a real format.
- New `admin_assignable_packages()` — replaces the bare `.from("platform_packages")` read `listAssignablePackages()` used, joining `plan_effective_price()` (1008) per row so a dynamic cycle's enabled discount is reflected. Legacy rows (`plan_id is null`) fall through to their own `price_minor` unchanged.
- `admin_gym_billing_history` (1010's fix) gains a `method` column and an optional `p_method` filter, dropped/recreated for the same signature-change reason.

**App:**
- `src/features/gyms/payment-method.ts` (new) — the shared `cash`/`upi`/`bank_transfer`/`card`/`cheque`/`other` vocabulary + `DEFAULT_PAYMENT_METHOD = "cash"`, used by both the client sheet and the server action's own validation before the RPC is ever called.
- `ManageSubscriptionSheet.tsx`'s Extend section gains an optional "Payment collected" amount + method (defaulting to Cash) — left blank, Extend behaves exactly as before (a free/goodwill extension); filled in, it's sent alongside the extend RPC call. Same single shared sheet implementation across the Gyms list, Gym Detail header, and Billing tab (three call sites, one file), so no duplicate wiring was needed.
- `listAssignablePackages()` (`features/gyms/queries.ts`) now returns both `price` (today's real payable price) and `listPrice` (the pre-discount price, only when they differ) — the "Change package" dropdown shows `"Growth · Monthly · ₹999 (was ₹1,499)"` when a discount is active.
- `features/gyms/billing.ts` + the Gym Detail Billing tab (`app/admin/gyms/[id]/billing/page.tsx`) gained a Method column and filter, and a "Manual" option on the existing Provider filter — so a cash-collected invoice is visible and filterable, not just present in the raw table.

**Verification, and its honest limit**: `tsc --noEmit` / `eslint .` / `npm run build` all clean (throwaway placeholder `.env.local`, deleted immediately, never committed — same pattern this file has used before). **This session had no Supabase MCP tool at all**, so unlike every other RPC-adding milestone in this file, there was no real-`.rpc()`-call verification against a disposable admin session before merging — the user applied `1014` directly via the SQL Editor on both DEV and PROD themselves and reported it done; this session could not independently confirm the apply succeeded, that the six touched functions execute without the 42702/42804-style defects the "Production incident: /admin/gyms 400s" write-up found the hard way in a structurally similar migration, or that a real extend-with-payment / change-package round trip behaves as designed. **A session with real DB access should run this project's own established disposable-admin verification** (extend with and without a payment attached, confirm the `platform_payments` row and both audit-log rows land correctly, change package onto a discounted dynamic cycle and confirm the RPC's `effective_price_minor` matches `plan_effective_price`'s own output, filter billing history by method) before this is fully trusted in production, per the two "Production incident" lessons above.
